    ...
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        configurations = mainFlake.outputs {
          ...
          kickstart-nix-nvim = { overlays.default = _: _: {}; };
          nix-index-database = {};
        };

        # Test that configurations build successfully
        buildTest = name: config: pkgs.runCommand "test-${name}-build" {} ''
        ...
        vmTest = name: configPath: pkgs.nixosTest {
          name = "test-${name}-vm";
          nodes.machine = { ... }: {
            imports = [ configPath ];
      ...
      in {
        checks = {
          ...
          # Script tests
          rebuild-script-test = pkgs.runCommand "test-rebuild-script" {
